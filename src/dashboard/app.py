...
from src.pipeline.module1_linker import (
    _extract_conclusion_text,
    _extract_published_endpoint,
    _extract_results_text,
)
...
TRIALS_COLUMNS = [
    ...
    "ctgov_pmid",
    "ctgov_publication_pmids",
    "ctgov_publication_types",
    "ctgov_publication_urls",
]
...
app_ui = ui.page_navbar(
    ...
    ui.nav_panel(
        ...
        ui.div(
            ...
            ui.div(
                card(
                    "Routing breakdown",
                    ui.p("How the AI classified each trial pair.", class_="text-muted text-small"),
                    output_widget("routing_overview_plot"),
                ),
                card(
                    "Review progress",
                    ui.p(
                        "How many pairs have been reviewed vs are still pending.",
    ...
    ui.nav_panel(
        ...
        ui.div(
            ...
            # ── Card set 1: Low-confidence links (pipeline found a candidate PMID) ──
            ui.tags.h4(
                "Low-confidence links — confirm or correct the PMID", style="margin: 24px 0 8px;"
            ),
            ui.p(
                "The pipeline found a candidate publication for each of these trials "
                "but could not verify it with high confidence. Review the proposed PMID, "
                "confirm or correct it, and save an audit note.",
                class_="text-muted text-small",
            ),
            ui.div(
                card(
                ...
                card("Selected trial: Linkage review", ui.output_ui("linkage_case_panel")),
                class_="grid-2",
            ),
            # ── Card set 2: no qualifying CT.gov-linked results paper ──
            ui.tags.h4(
                "Trials with no qualifying linked results paper", style="margin: 32px 0 8px;"
            ),
            ui.p(
                "ClinicalTrials.gov listed no PMID, or none of its linked articles passed the "
                "results-paper classifier. Search PubMed manually if needed; enter the PMID and set "
                "confidence to High or Medium. If no publication exists, leave the PMID blank, "
                "select 'Unlinked', and note the reason (e.g. 'Results not yet published').",
                class_="text-muted text-small",
            ),
            ui.div(
                card(
                    "No-publication queue",
                    ui.output_ui("unlinked_selector"),
                    ui.tags.hr(),
                    ui.input_text("unlinked_reviewer_initials", "Your initials"),
                    ui.output_ui("unlinked_feedback"),
                    ui.tags.hr(),
                    ui.output_data_frame("unlinked_queue_table"),
                ),
                card("Selected trial: No-publication review", ui.output_ui("unlinked_case_panel")),
                class_="grid-2",
            ),
            card(
                "Full Linkage Audit Log: All NCTs and Linked PMIDs",
                ui.p(
                    "Every NCT ID and selected PMID, with all CT.gov candidate verdicts in "
                    "the notes. Use this to inspect trials linked to multiple publications.",
                    class_="text-muted text-small",
                ),
        ...
        ui.div(
            ...
            ui.div(
                card(
                    ...
                    ui.tags.hr(),
                    ui.p(
                        "Your initials are added to the audit log with every decision.",
                        class_="text-muted text-small",
                    ),
    ...
    ui.nav_panel(
        ...
        ui.div(
            ...
            card(
                ...
                ui.div(
                    ui.input_selectize(
                        "filter_routing", "Routing method", ROUTING_LABELS, multiple=True
                    ),
                    ui.input_selectize(
                        "filter_confidence",
                        "Model confidence",
            ...
            card(
                ...
                ui.div(
                    ...
                    ui.download_button(
                        "download_decision_log_filtered",
        ...
        ui.div(
            ...
            ui.div(
                card(
                    ...
                    ui.p(
                        "Each bar is one pair. Green dashed line = auto-concordant threshold (0.90). "
                ...
                card(
                    "Routing split",
                    ui.p(
                        "Share of pairs handled by each classification method.",
                        class_="text-muted text-small",
                    ),
    ...
    ui.nav_panel(
        ...
        ui.div(
            ...
            card(
                "Routing vs final classification",
                ui.p(
                    "Each cell shows how many pairs were routed a certain way and "
    ...
    ui.nav_panel(
        ...
        ui.div(
            ...
            ui.div(
                card(
                    "Assumed vs evidence-based HR",
                    ui.p(
                        "Each dot is one trial. X axis = what the evidence said at the "
                ...
                card(
                    "Optimism bias distribution",
                    ui.p(
                        "Optimism bias = assumed HR minus posterior HR. "
        ...
        ui.div(
            ...
            card(
                ...
                ui.div(
                    ...
                    ui.download_button(
                        "download_decision_log_raw",
                        "Decision log (.csv)",
                        class_="btn btn-secondary",
                    ),
                    ui.download_button(
                        "download_linkage_log_raw", "Linkage log (.csv)", class_="btn btn-secondary"
                    ),
                    ui.download_button(
                        "download_power_audit_raw",
                        "Power audit log (.csv)",
                        class_="btn btn-secondary",
                    ),
                    class_="button-strip",
    ...
    header=ui.div(
        ui.div(
            ui.input_action_button(
                "refresh_data", "Reload data", class_="btn btn-secondary btn-sm"
            ),
            class_="app-toolbar",
        ),
        head_content(ui.include_css(CSS_PATH, method="inline")),
    ),
)
...
def server(input: Inputs, output: Outputs, session: Session) -> None:
    ...
    def linked_trials_frame() -> pd.DataFrame:
        ...
        df = read_csv_or_empty(
            ...
            + [
                ...
                "primary_result_candidate_details",
                "interim_stage_pmids",
                "primary_results_pmids",
                "final_results_pmid",
                "final_results_pmids",
                "interim_results_pmids",
                "updated_results_pmids",
                "secondary_results_pmids",
                "subgroup_pmids",
                "safety_pmids",
                "qol_pmids",
                "biomarker_pmids",
                "long_term_followup_pmids",
                "extension_study_pmids",
                "protocol_sap_pmids",
                "other_pmids",
                "unresolved_pmids",
                "linkage_method",
                ...
                "pub_year",
                "journal",
                "registry_endpoint_source",
                "registry_history_status",
                "registry_versions_retrieved",
                "endpoint_switch_assessable",
                "registry_history_url",
                "linkage_notes",
                "linkage_flag",
            ],
        )
        if not df.empty and "nct_id" in df.columns:
            df = df.drop_duplicates(subset=["nct_id"], keep="last")
        return df
    ...
    def _trial_meta(nct_id: str, pair_pmid: str = "") -> dict:
        ...
        if not lf.empty:
            linked = lf.loc[
                (lf["nct_id"] == nct_id) & ((lf["pmid"] == pair_pmid) if pair_pmid else True)
            ]
            if not linked.empty:
                row = linked.iloc[0]
                return {
                    ...
                    "abstract_text": str(row.get("abstract_text", "")).strip(),
                    "published_results": str(row.get("published_results", "")).strip(),
                    "published_conclusion": str(row.get("published_conclusion", "")).strip(),
                }
    ...
    def linkage_queue_frame() -> pd.DataFrame:
        """Low-confidence links: pipeline found a candidate PMID but couldn't verify it."""
        linked = linked_trials_frame()
        if linked.empty:
            return linked
        flagged = linked[linked["linkage_confidence"] == "Low"].copy()
        flagged = flagged.drop_duplicates(subset=["nct_id"], keep="last")
        return flagged.sort_values("nct_id").reset_index(drop=True)

    @reactive.calc
    def unlinked_queue_frame() -> pd.DataFrame:
        """Trials without a qualifying CT.gov-linked results publication."""
        linked = linked_trials_frame()
        if linked.empty:
            return linked
        flagged = linked[
            (linked["linkage_confidence"] == "Unlinked")
            & (linked["linkage_flag"].fillna("NO_PUBLICATION_FOUND") != "NO_PUBLICATION_CONFIRMED")
        ].copy()
        flagged = flagged.drop_duplicates(subset=["nct_id"], keep="last")
        return flagged.sort_values("nct_id").reset_index(drop=True)
    ...
    def reviewed_frame() -> pd.DataFrame:
        ...
        return frame.sort_values("_ts", ascending=False, na_position="last").drop(columns=["_ts"])
    ...
    @reactive.calc
    def selected_unlinked_row() -> pd.Series | None:
        queue = unlinked_queue_frame()
        if queue.empty:
            return None
        nct_id = safe_input(input.selected_unlinked_nct, "")
        if nct_id in queue["nct_id"].values:
            return queue.loc[queue["nct_id"] == nct_id].iloc[0]
        return queue.iloc[0]
    ...
    def _refresh_matched_outputs(linked_frame: pd.DataFrame, nct_id: str, pmid: str) -> None:
        ...
        candidate = linked_frame[
            (linked_frame["nct_id"] == nct_id) & (linked_frame["pmid"] == pmid)
        ].copy()
        if candidate.empty:
            return
        ...
        refreshed = linked_frame.drop(
            columns=[
                col
                for col in ["pair_id", "similarity_score", "routing"]
                if col in linked_frame.columns
            ],
            errors="ignore",
        ).merge(
            updates,
    ...
    def _submit_linkage_review() -> None:
        ...
        if not notes:
            linkage_feedback_state.set(
                ("danger", "Add a short audit note for the manual linkage decision.")
            )
            return
        ...
        if linked.empty or nct_id not in linked["nct_id"].values:
            linkage_feedback_state.set(
                ("danger", "Could not load linked_trials.csv for this case.")
            )
            return
        ...
        linked.loc[idx, "primary_result_status"] = (
            "SELECTED" if chosen_conf in {"High", "Medium"} else "NOT_FOUND"
        )
        linked.loc[idx, "primary_results_pmid"] = pmid if chosen_conf in {"High", "Medium"} else ""
        linked.loc[idx, "abstract_text"] = abstract_text
        linked.loc[idx, "published_results"] = published_results
        linked.loc[idx, "published_conclusion"] = published_conclusion
        ...
        linked.loc[idx, "pub_date"] = pub_date
        linked.loc[idx, "journal"] = journal
        linked.loc[idx, "linkage_notes"] = notes + fetch_warning
        linked.loc[idx, "linkage_flag"] = (
            "" if chosen_conf in {"High", "Medium"} else "FLAGGED_FOR_REVIEW"
        )
    ...
    def linkage_intro() -> ui.Tag:
        n_low = len(linkage_queue_frame())
        n_unlinked = len(unlinked_queue_frame())
        if n_low == 0 and n_unlinked == 0:
            return info_box(
                "All trial-publication links are resolved to High or Medium confidence. "
        ...
        if n_low:
            parts.append(
                f"{n_low} trial(s) have a candidate PMID the pipeline could not verify with high "
                "confidence — review and confirm below."
            )
        if n_unlinked:
            parts.append(
                f"{n_unlinked} trial(s) have no linked publication at all. "
                "Search PubMed manually and either enter the PMID or confirm that no results paper exists."
            )
        return info_box(" ".join(parts), "warning")
    ...
    def linkage_kpis() -> ui.Tag:
        ...
        if frame.empty:
            return ui.div(
                kpi_box("Trials fetched", "0", "Run Module 1 to populate"),
                ...
                kpi_box("Low confidence", "0", "Candidate found, needs review", "warning"),
                kpi_box("No results paper", "0", "No qualifying CT.gov-linked PMID", "danger"),
                class_="kpi-grid",
            )
        return ui.div(
            kpi_box("Trials fetched", str(len(frame)), "Rows in linked_trials.csv"),
            kpi_box(
                "High confidence",
                str(len(frame[frame["linkage_confidence"] == "High"])),
                "Direct or confirmed links",
                "success",
            ),
            kpi_box(
                "Medium confidence",
                str(len(frame[frame["linkage_confidence"] == "Medium"])),
                "Usable after disambiguation",
                "accent",
            ),
            kpi_box(
                "Low confidence",
                str(len(linkage_queue_frame())),
                "Candidate found — confirm or correct PMID",
                "warning",
            ),
            kpi_box(
                "No results paper",
                str(len(unlinked_queue_frame())),
                "No qualifying CT.gov-linked PMID — review needed",
                "danger",
            ),
            class_="kpi-grid",
        )
    ...
    def linkage_selector() -> ui.Tag:
        ...
        choices = {
            str(
                row["nct_id"]
            ): f"{row['nct_id']}: {safe_text(row.get('official_title') or row.get('brief_title'), 'Untitled trial')[:80]}"
    ...
    def linkage_feedback() -> ui.Tag:
        ...
        tone, message = state
        return info_box(
            message, tone if tone in {"info", "success", "warning", "danger"} else "info"
        )
    ...
    def linkage_queue_table() -> pd.DataFrame:
        ...
        out = queue[
            ["nct_id", "pmid", "linkage_confidence", "linkage_method", "linkage_notes"]
        ].copy()
    ...
    def linkage_case_panel() -> ui.Tag:
        ...
        title = safe_text(
            row.get("official_title") or row.get("brief_title"), "Title not available"
        )
        ctgov_url = f"https://clinicaltrials.gov/study/{nct_id}"
        current_pmid = str(row.get("pmid", "")).strip()
        current_pubmed_url = (
            f"https://pubmed.ncbi.nlm.nih.gov/{current_pmid}/" if current_pmid else ""
        )

        header_links = [
            ui.tags.a(nct_id, href=ctgov_url, target="_blank", class_="trial-link ctgov-link")
        ]
        if current_pmid:
            header_links.append(
                ui.tags.a(
                    f"PMID {current_pmid}",
                    href=current_pubmed_url,
                    target="_blank",
        ...
        return ui.div(
            ui.div(
                ...
                ui.div(
                    ui.span(
                        f"Current confidence: {safe_text(row.get('linkage_confidence'), 'Not set')}",
                        class_="trial-badge",
                    ),
                    ui.span(
                        f"Current method: {safe_text(row.get('linkage_method'), 'Not set')}",
                        class_="trial-badge",
                    ),
                    ui.span(
                        "Primary-paper status: "
                        f"{safe_text(row.get('primary_result_status'), 'Not set')}",
                        class_="trial-badge",
                    ),
                    ui.span(
                        f"Family size: {safe_text(row.get('publication_family_count'), '0')}"
                        + (
                            f" ({safe_text(row.get('publication_family_rejected_count'), '0')} rejected)"
                            if safe_text(row.get("publication_family_rejected_count"), "0")
                            not in ("0", "")
                            else ""
                        ),
                        class_="trial-badge",
                    ...
                    ui.span(
                        "Endpoint switch assessable: "
                        f"{safe_text(row.get('endpoint_switch_assessable'), 'Unknown')}",
                        class_="trial-badge",
                    ),
                    ui.span(
                        "CT.gov publication PMIDs: "
                        f"{safe_text(row.get('ctgov_publication_pmids') or row.get('ctgov_pmid'), 'Not available')}",
                        class_="trial-badge",
                    ),
                    class_="trial-badges",
                ),
            ...
            info_box(
                "Primary candidates: "
                f"{safe_text(row.get('primary_result_candidates'), 'None classified')}. "
                "Confirm a PMID only if it reports the prespecified primary analysis of "
                "the complete randomized population. Final, interim, subgroup, safety, "
                "and QoL papers remain separate members of the publication family.",
                "warning",
            ),
            ui.input_text(
                "linkage_pmid",
                "Confirmed primary-results PMID",
                value=current_pmid,
            ...
            ui.input_action_button(
                "submit_linkage_review", "Save linkage review", class_="btn btn-primary"
            ),
            class_="detail-stack",
        )

    # ── Unlinked queue renderers ──────────────────────────────────────────

    @render.ui
    def unlinked_selector() -> ui.Tag:
        queue = unlinked_queue_frame()
        if queue.empty:
            return info_box(
                "No unlinked trials — all trials have at least a candidate PMID.", "success"
            )
        choices = {
            str(row["nct_id"]): (
                f"{row['nct_id']}: "
                f"{safe_text(row.get('official_title') or row.get('brief_title'), 'Untitled trial')[:80]}"
            )
            for _, row in queue.iterrows()
        }
        return ui.input_selectize(
            "selected_unlinked_nct",
            "Select a trial to review",
            choices,
            selected=queue.iloc[0]["nct_id"],
        )
    ...
    @render.data_frame
    def unlinked_queue_table() -> pd.DataFrame:
        queue = unlinked_queue_frame()
        if queue.empty:
            return pd.DataFrame(columns=["nct_id", "phase", "completion_date", "linkage_notes"])
        cols = ["nct_id", "phase", "completion_date", "linkage_notes"]
        available = [c for c in cols if c in queue.columns]
        out = queue[available].copy()
        return out.reset_index(drop=True)

    @render.ui
    def unlinked_case_panel() -> ui.Tag:
        row = selected_unlinked_row()
        if row is None:
            return info_box("No unlinked trials to review.", "success")
        ...
        ctgov_url = f"https://clinicaltrials.gov/study/{nct_id}"
        pubmed_search_url = f"https://pubmed.ncbi.nlm.nih.gov/?term={nct_id}+OR+" + "+".join(
            title.split()[:6]
        )
        ...
        return ui.div(
            ui.div(
                ui.div(
                    ui.tags.a(
                        nct_id, href=ctgov_url, target="_blank", class_="trial-link ctgov-link"
                    ),
                    ui.tags.a(
                        "Search PubMed →",
                        href=pubmed_search_url,
                        target="_blank",
                        class_="trial-link pubmed-link",
                    ),
                    class_="trial-id-row",
                ...
                ui.div(
                    ui.span(
                        f"Phase: {safe_text(row.get('phase'), 'Not recorded')}",
                        class_="trial-badge",
                    ),
                    ui.span(
                        f"Completed: {safe_text(row.get('completion_date'), 'Not recorded')}",
                        class_="trial-badge",
                    ),
                    ui.span(
                        f"Results posted on CT.gov: {safe_text(row.get('results_first_posted_date'), 'No')}",
                        class_="trial-badge",
                    ),
                    ui.span(
                        "CT.gov publication PMIDs: "
                        f"{safe_text(row.get('ctgov_publication_pmids') or row.get('ctgov_pmid'), 'Not listed')}",
                        class_="trial-badge",
                    ),
                    class_="trial-badges",
                ),
            ...
            info_box(
                "No article was verified as the complete primary analysis. "
                "Review the publication family, CT.gov References, and PubMed search above. "
                "If you confirm the primary-results paper: enter its PMID and set confidence to "
                "High or Medium. Do not enter an interim, final-update, subgroup, safety, or QoL "
                "paper in its place. "
                "If no results paper exists yet (or ever): leave the PMID blank, keep status as Unlinked, "
                "and write a note (e.g. 'Results not published', 'Terminated early, no publication').",
                "warning",
            ),
            info_box(safe_text(row.get("linkage_notes"), "No pipeline notes recorded."), "info"),
            ui.input_text(
                "unlinked_pmid",
                "PMID found (leave blank if no publication exists)",
                value="",
                placeholder="e.g. 29130810",
            ),
            ui.input_select(
                "unlinked_confidence_choice",
                "Final linkage status",
                {
                    "Unlinked": "Unlinked — no results paper found / not yet published",
                    "High": "High — I found and verified the results paper",
                    "Medium": "Medium — I found a likely match but cannot fully confirm",
                },
                selected="Unlinked",
            ),
            ui.input_text_area(
                ...
                "Audit note (required)",
                rows=4,
                placeholder="e.g. 'Results not yet published as of 2026-06.' OR 'Found PMID 12345678 via CT.gov references section.'",
            ),
            ui.input_action_button(
                "submit_unlinked_review", "Save review", class_="btn btn-primary"
            ),
            class_="detail-stack",
        )

    @reactive.effect
    @reactive.event(input.submit_unlinked_review)
    def _submit_unlinked_review() -> None:
        row = selected_unlinked_row()
        if row is None:
            linkage_feedback_state.set(("warning", "No trial selected."))
            return

        reviewer = str(safe_input(input.unlinked_reviewer_initials, "")).strip().upper()
        chosen_conf = str(safe_input(input.unlinked_confidence_choice, "")).strip()
        pmid = str(safe_input(input.unlinked_pmid, "")).strip()
        notes = str(safe_input(input.unlinked_notes, "")).strip()
        nct_id = str(row.get("nct_id", "")).strip()
        ...
        if chosen_conf != "Unlinked" and not pmid:
            linkage_feedback_state.set(
                ("danger", "Enter the PMID for a High or Medium resolution.")
            )
            return
        if not notes:
            linkage_feedback_state.set(
                ("danger", "An audit note is required for all no-publication reviews.")
            )
            return

        linked = linked_trials_frame().copy()
        if linked.empty or nct_id not in linked["nct_id"].values:
            linkage_feedback_state.set(("danger", "Could not load linked_trials.csv."))
            return

        fetch_warning = ""
        abstract_text = published_results = published_conclusion = ""
        published_endpoint = first_author = pub_year = pub_date = journal = ""
        ...
        if pmid and chosen_conf != "Unlinked":
            try:
                ...
                fetch_warning = f" PubMed fetch failed: {exc}"
        ...
        if chosen_conf in {"High", "Medium"}:
            linked.loc[idx, "linkage_flag"] = ""
        else:
            linked.loc[idx, "linkage_flag"] = "NO_PUBLICATION_CONFIRMED"
        ...
        if chosen_conf in {"High", "Medium"} and pmid:
            try:
                ...
                linkage_feedback_state.set(
                    (
                        "warning",
                        f"Saved for {nct_id} but endpoint matching did not refresh: {exc}",
                    )
                )
                refresh_counter.set(refresh_counter.get() + 1)
                return
        ...
        refresh_counter.set(refresh_counter.get() + 1)
        if chosen_conf in {"High", "Medium"}:
            msg = f"Linked {nct_id} → PMID {pmid} ({chosen_conf}). Pushed to endpoint matching."
        else:
            msg = f"Confirmed no publication for {nct_id}. Audit note saved."
        if fetch_warning:
            msg += " PubMed extraction needs manual follow-up."
        linkage_feedback_state.set(("success", msg))
    ...
    def overview_kpis() -> ui.Tag:
        ...
        return ui.div(
            ...
            kpi_box(
                "Poolable", str(poolable), "Pairs confirmed eligible for meta-analysis", "accent"
            ),
            class_="kpi-grid",
        )
    ...
    def overview_guidance() -> ui.Tag:
        ...
        switches = frame["llm_switch_type"].isin(["moderate_switch", "major_switch"]).sum()
        auto_accepted = (frame["human_reviewed"] == "auto_accepted").sum()
        no_pub_ep = len(frame[frame["published_endpoint"].eq("")])

        lines = [
            f"The pipeline classified {len(frame)} breast cancer trial-publication pairs "
            "(every pair adjudicated by the LLM).",
            f"{switches} pair(s) were classified as an outcome switch — all sent to human review.",
            (
                "Automatic acceptance is disabled: every model classification requires human review."
                if not ENDPOINT_AUTO_ACCEPT
                else f"{auto_accepted} confident non-switch verdict(s) were auto-accepted "
                "(a spot-check sample is still reviewed)."
            ),
        ]
        if no_pub_ep:
            lines.append(
        ...
        if pending > 0:
            lines.append(f"{pending} pair(s) remain in the review queue.")
        else:
    ...
    def runtime_configuration() -> ui.Tag:
        ...
        rate = len(reviewed[reviewed["human_decision"] == "override"]) / max(len(reviewed), 1)
        ...
        if rate > OVERRIDE_RATE_HIGH_THRESHOLD:
            note = f"{rate:.0%} (high: endpoint adjudicator requires recalibration)"
        ...
        return kv(
            ...
            "enabled" if ENDPOINT_AUTO_ACCEPT else "disabled during recalibration",
            "Override rate",
            note,
        )
    ...
    def routing_overview_plot():
        ...
        counts = frame["routing"].replace(ROUTING_LABELS).value_counts().reset_index()
    ...
    def review_queue_intro() -> ui.Tag:
        ...
        spot_checks = sum(
            is_spot_check_row(row, decision_log_frame()) for _, row in queue.iterrows()
        )
        return info_box(
    ...
    def review_queue_kpis() -> ui.Tag:
        frame = decision_log_frame()
        n_auto = int((frame["human_reviewed"] == "auto_accepted").sum())
        return ui.div(
            kpi_box(
                "Pending",
                str(len(queue_frame())),
                "Every model verdict requires review"
                if not ENDPOINT_AUTO_ACCEPT
                else "Flagged / low-confidence / spot-check",
                "warning",
            ),
            kpi_box(
                "Human-reviewed",
                str(len(reviewed_frame())),
                "Confirmed or overridden by a person",
                "success",
            ),
            kpi_box(
                "Auto-accepted",
                str(n_auto),
                "Disabled during model recalibration"
                if not ENDPOINT_AUTO_ACCEPT
                else "Confident, unflagged model verdicts",
                "accent",
            ),
            kpi_box(
                "Missing endpoint",
                str(len(frame[frame["published_endpoint"].eq("")])),
                "No published endpoint text — needs manual extraction.",
                "danger",
            ),
            class_="kpi-grid",
        )
    ...
    def review_selector() -> ui.Tag:
        ...
        return ui.input_selectize(
            "selected_review_pair",
            "Select a pair to review",
            choices,
            selected=queue.iloc[0]["pair_id"],
        )
    ...
    def recent_reviews_table() -> pd.DataFrame:
        ...
        if reviewed.empty:
            return pd.DataFrame(
                columns=["pair_id", "decision", "classification", "reviewer", "timestamp"]
            )
        out = (
            reviewed[
                [
    ...
    def review_case_panel() -> ui.Tag:
        ...
        abstract_text = meta.get("abstract_text", "")
        published_results = meta.get("published_results", "")
        published_conclusion = meta.get("published_conclusion", "")
        ...
        if routing == "llm":
            review_triggers.append(
                "The five-class adjudicator reviewed this pair; similarity is informational only."
            )
        if safe_text(row.get("llm_adjudication_guardrail"), ""):
            review_triggers.append(
                "A deterministic guardrail corrected an unsupported model classification: "
                + str(row.get("llm_adjudication_guardrail"))
            )
        if str(row.get("llm_confidence", "")).lower() == "low":
            review_triggers.append(
                "The AI model self-reported low confidence, so human review is mandatory."
            )
        ...
        if not str(row.get("published_endpoint", "")).strip():
            review_triggers.append(
                "No published endpoint text could be extracted from the linked PubMed/PMC record."
            )
        if is_spot_check_row(row, decision_log_frame()):
            review_triggers.append(
                "This pair was selected in the proposal's 10% spot-check sample for auto-routed decisions."
            )
        ...
        if routing == "auto_major_switch":
            ...
            routing_note = (
                f"Similarity score {score} is informational only. The five-class model adjudicated this pair. "
                + (
                    f"AI classified as: {ai_class}."
                    if ai_class
        ...
        pub_ep_display = (
            ...
            if pub_ep
                ...
                + (
                    ...
                    if pmid
                    else f"Check the trial record directly: {nct_id}."
                )
            )
        )
        ...
        if abstract_text:
            publication_blocks.append(
                ui.div(
                    ui.div("Abstract", class_="endpoint-source"),
                    ui.div(abstract_text, class_="endpoint-text"),
                    class_="endpoint-box",
                )
            )
        if published_results:
            publication_blocks.append(
                ui.div(
                    ui.div("Results", class_="endpoint-source"),
                    ui.div(published_results, class_="endpoint-text"),
                    class_="endpoint-box",
                )
            )
        if published_conclusion:
            publication_blocks.append(
                ui.div(
                    ui.div("Conclusion", class_="endpoint-source"),
                    ui.div(published_conclusion, class_="endpoint-text"),
                    class_="endpoint-box",
                )
            )

        endpoint_comparison_blocks = []
        raw_comparisons = str(row.get("llm_endpoint_comparisons", "") or "").strip()
        try:
            endpoint_comparisons = json.loads(raw_comparisons) if raw_comparisons else []
        except (TypeError, ValueError, json.JSONDecodeError):
            endpoint_comparisons = []
        if isinstance(endpoint_comparisons, list):
            attribute_labels = {
                "same_construct": "Clinical construct",
                "same_timeframe": "Timeframe",
                "same_definition": "Definition",
                "same_population": "Population",
                "same_measurement_method": "Assessment method",
            }

            def comparison_status(value: object) -> str:
                if value is True:
                    return "same"
                if value is False:
                    return "different"
                return "not stated / unclear"

            for number, comparison in enumerate(endpoint_comparisons, start=1):
                if not isinstance(comparison, dict):
                    continue
                statuses = "; ".join(
                    f"{label}: {comparison_status(comparison.get(field))}"
                    for field, label in attribute_labels.items()
                )
                evidence = comparison.get("actual_change_evidence") or []
                if isinstance(evidence, list):
                    evidence = " | ".join(str(item) for item in evidence if str(item).strip())
                endpoint_comparison_blocks.append(
                    ui.div(
                        ui.div(f"Registered primary endpoint {number}", class_="endpoint-source"),
                        ui.div(
                            safe_text(comparison.get("registered_endpoint"), "Not extracted"),
                            class_="endpoint-text",
                        ),
                        ui.div(
                            "Published counterpart: "
                            + safe_text(
                                comparison.get("published_corresponding_endpoint"),
                                "Not reported",
                            ),
                            class_="endpoint-text",
                        ),
                        ui.div(statuses, class_="endpoint-text"),
                        ui.div(
                            "Endpoint classification: "
                            + safe_text(comparison.get("classification_label"), "Not classified"),
                            class_="endpoint-text",
                        ),
                        ui.div(
                            "Change evidence: "
                            + safe_text(evidence, "None; missing detail is not a change."),
                            class_="endpoint-text",
                        ),
                        class_="endpoint-box",
                    )
        ...
        return ui.div(
            ...
            ui.div(
                ui.div(
                    ui.div(
                        "Registered endpoint (ClinicalTrials.gov, pre-specified)",
                ...
                ui.div(
                    ui.div(
                        "Published endpoint (PubMed / PMC abstract or full text)",
                        class_="endpoint-source",
                    ),
            ...
            ui.div(*publication_blocks, class_="endpoint-grid") if publication_blocks else ui.div(),
            ui.div(*endpoint_comparison_blocks, class_="endpoint-grid")
            if endpoint_comparison_blocks
            else ui.div(),
            # AI signals
            ui.div(
                ...
                ui.div(
                    f"AI classification: {SWITCH_LABELS.get(str(row.get('llm_switch_type')), 'Not classified')}",
                    class_="signal-item",
                ),
                ui.div(
                    f"AI confidence: {safe_text(row.get('llm_confidence'), 'Not called')}",
                    class_="signal-item",
                ),
                ui.div(
                    "Endpoint match: "
                    f"{safe_text(row.get('llm_primary_endpoint_match'), 'Not reported')}",
                    class_="signal-item",
                ),
                class_="signals-grid",
            ),
            ui.div(
                ui.div("Model-extracted published primary", class_="endpoint-source"),
                ui.div(
                    safe_text(row.get("llm_published_primary_extracted"), "Not reported."),
                    class_="endpoint-text",
                ),
                ui.div("Affirmative change evidence", class_="endpoint-source"),
                ui.div(
                    safe_text(
                        row.get("llm_actual_change_evidence"),
                        "None supplied. Missing abstract detail cannot establish a change.",
                    ),
                    class_="endpoint-text",
                ),
                class_="endpoint-box",
            ...
            *_reasoning_block(row),
            ...
            ui.input_action_button("submit_review", "Save decision", class_="btn btn-primary"),
            class_="detail-stack",
        )
    ...
    @render.download(filename=lambda: f"decision-log-{datetime.now(UTC).date().isoformat()}.csv")
    ...
    def calibration_intro() -> ui.Tag:
        ...
        return info_box(
            ...
            "The similarity score histogram is descriptive only; similarity does not determine the five-class verdict. "
    ...
    def calibration_kpis() -> ui.Tag:
        ...
        inter_rater = inter_rater_frame()
        calibration = (
            compute_ai_calibration(frame, gold) if (not frame.empty and not gold.empty) else {}
        )
        kappa_metrics = (
        ...
        return ui.div(
            kpi_box(
                ...
                fmt_pct(len(frame[frame["routing"] == "llm"]), len(frame)),
                "Share of pairs sent to the AI model",
                "accent",
            ),
            kpi_box(
                "Gold AUC", fmt_num(calibration.get("auc"), 3, "-"), "Target > 0.80", "primary"
            ),
            kpi_box(
                "Precision (mod+)",
                fmt_num(calibration.get("precision_moderate_or_above"), 3, "-"),
                "Target > 0.75",
                "success",
            ),
            kpi_box(
                "Recall (mod+)",
                fmt_num(calibration.get("recall_moderate_or_above"), 3, "-"),
                "Target > 0.80",
                "warning",
            ),
            kpi_box(
                "Inter-rater kappa",
                fmt_num(kappa_metrics.get("cohen_kappa"), 3, "-"),
                "Second-reviewer agreement",
                "accent",
            ),
            kpi_box(
                "Reviewed",
                fmt_pct(len(reviewed), len(frame)),
                "Share of log that has been human-reviewed",
                "success",
            ),
            class_="kpi-grid",
        )
    ...
    def similarity_plot():
        ...
        fig.add_vline(
            x=0.90, line_dash="dash", line_color="#15896b", annotation_text="Auto-concordant (0.90)"
        )
        fig.add_vline(
            x=0.50,
            line_dash="dash",
    ...
    def gold_matrix() -> pd.DataFrame:
        ...
        merged["ai_switch"] = merged["llm_switch_type"].replace("", "concordant")
        return (
            pd.crosstab(merged["ai_switch"], merged["gold_switch_type"])
            .rename_axis("AI →  / Gold ↓")
            .reset_index()
        )
    ...
    def scorecard_kpis() -> ui.Tag:
        frame = decision_log_frame()
        reviewed = reviewed_frame()
        switched = reviewed[reviewed["human_final_class"].isin(["moderate_switch", "major_switch"])]
        agreed = reviewed[reviewed["llm_switch_type"].eq(reviewed["human_final_class"])]
        return ui.div(
            kpi_box("Reviewed", fmt_pct(len(reviewed), len(frame)), "Of all pairs", "accent"),
            kpi_box(
                "Confirmed switches",
                fmt_pct(len(switched), len(reviewed)),
                "Endpoint changed between registration and publication",
                "warning",
            ),
            kpi_box(
                "AI-human agreement",
                fmt_pct(len(agreed), len(reviewed)),
                "AI and reviewer agreed on classification",
                "success",
            ),
            kpi_box(
                "Poolable",
                str(len(frame[frame["human_poolable"].map(truthy)])),
                "Approved for meta-analysis",
                "primary",
            ),
            class_="kpi-grid",
        )
    ...
    def scorecard_heatmap():
        ...
        if plot.empty:
            return empty_figure("Routing vs classification", "No reviews saved yet.")
        pivot = pd.crosstab(
            plot["routing"].replace(ROUTING_LABELS),
    ...
    def power_kpis() -> ui.Tag:
        ...
        return ui.div(
            kpi_box("Trials audited", str(len(frame)), "Rows in power audit log"),
            kpi_box("Included", str(len(frame) - len(excluded)), "Valid inputs", "success"),
            kpi_box("Excluded", str(len(excluded)), "Missing inputs", "warning"),
            kpi_box(
                "Mean bias",
                fmt_num(bias.mean(), 3, "-"),
                "Assumed HR − posterior HR. Positive = optimistic",
                "accent",
            ),
            class_="kpi-grid",
        )
    ...
    def power_scatter_plot():
        ...
        fig = px.scatter(
            plot,
            x="posterior_hr_at_registration",
            y="assumed_hr",
            hover_data=["nct_id"],
            labels={
                "posterior_hr_at_registration": "Evidence-based HR at registration",
    ...
    def power_bias_plot():
        ...
        fig.add_vline(x=0, line_dash="dash", line_color="#7f8c8d", annotation_text="Zero bias")
        return fig.update_layout(margin=dict(l=10, r=10, t=10, b=10))
    ...
    def power_excluded_table() -> pd.DataFrame:
        ...
        out = frame[frame["excluded_reason"].ne("")][["nct_id", "excluded_reason"]].copy()
    ...
    def provenance_panel() -> ui.Tag:
        ...
        row = (
            frame.loc[frame["pair_id"] == pair_id].iloc[0]
            if pair_id in frame["pair_id"].values
            else frame.iloc[0]
        )
        ...
        return ui.div(
            kv(
                ...
                linkage_text,
                "Registered endpoint",
                safe_text(row.get("registered_endpoint")),
                "Published endpoint",
                safe_text(row.get("published_endpoint"), "None extracted"),
                "Similarity score",
                fmt_num(row.get("similarity_score"), 4),
                "Routing",
                ROUTING_LABELS.get(str(row.get("routing")), safe_text(row.get("routing"))),
                "AI classification",
                SWITCH_LABELS.get(str(row.get("llm_switch_type")), "Not classified"),
                "AI confidence",
                safe_text(row.get("llm_confidence"), "Not called"),
                "Human decision",
                safe_text(row.get("human_decision"), "Not reviewed"),
                "Human final class",
                SWITCH_LABELS.get(str(row.get("human_final_class")), "Pending review"),
                "Poolable",
                "Yes" if truthy(row.get("human_poolable")) else "No",
                "Reviewer",
                safe_text(row.get("reviewer_initials"), "Not yet reviewed"),
                "Override reason",
                safe_text(row.get("override_reason"), "No override"),
                "Review timestamp",
                safe_text(row.get("review_timestamp"), "Not yet reviewed"),
            ),
    ...
    def export_governance() -> ui.Tag:
        ...
        return kv(
            "Total pairs",
            str(summary.get("total_pairs", 0)),
            "Model adjudication rate",
            f"{summary.get('llm_call_rate_pct', 0)}%",
            "Human review rate",
            f"{summary.get('human_review_rate_pct', 0)}%",
            "Human override rate",
            f"{summary.get('human_override_rate_pct', 0)}%",
        )
    ...
    def download_audit_package() -> bytes:
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as arc:
            for name, path in {
                ...
                "publication_family.csv": PUBLICATION_FAMILY_PATH,
                "registry_history.csv": REGISTRY_HISTORY_PATH,
    ...
    def download_linkage_log_raw() -> str:
        return (
            LINKAGE_LOG_PATH.read_text(encoding="utf-8")
    ...
    def download_power_audit_raw() -> str:
        return (
            POWER_AUDIT_LOG_PATH.read_text(encoding="utf-8")
